...
function flipCard() {
    if (firstCard) {
        ...
        // Check for a match
        foundMatch = checkMatch(matches[card1.dataset.num],matches[card2.dataset.num]);
        if (foundMatch) {
            foundMatches++;
            score += 25;
...
function updateScore() {
    ...
    if (numMatches === foundMatches) {
        scoreDisplay.textContent = "You win! It took you " + turn + " turns and your final score was " + score;
        promptForRestart();
    } else {
        scoreDisplay.textContent = "You've taken " + turn + " turns. Your score is: " + score;
    }    
}
